    ...
    public class Sha512
    {
        ...
        public void Update(byte[] data, int index, int length)
        {
            ...
            if (!((index + length) <= data.Length))
                throw new ArgumentException("index + length must be less then data lenght");
        ...
        {
            if (output == null || output.Count != 64)
                throw new ArgumentException("output must have 64 bytes");
