...
namespace AsmodatStandard.Cryptography
{
    ...
    public static class XXHash
    {
        ...
        /// <summary>
        /// XXHash only supports 32 bit and 64 bit outputs. To correctly generate the 128 bit hash, we hash the same phrase twice, with seed 0 and seed 1, and concatenate them
        /// This method is usefull in case of Parity Substrate key hashing
        /// </summary>
        /// <param name="input"></param>
        /// <returns></returns>
        public static unsafe byte[] Hash128s01(string input, bool LE = true) => Hash128s01(Encoding.UTF8.GetBytes(input), LE: LE);

        public static unsafe byte[] Hash128s01(byte[] buffer, bool LE = true)
        {
            var al = Hash64(buffer, LE: LE, seed: 0);
            var ar = Hash64(buffer, LE: LE, seed: 1);
            return al.Merge(ar);
        }

        public static unsafe byte[] Hash64(byte[] input, bool LE, ulong seed = 0)
        {
            var h = Hash64(input, seed: seed);
            var a = BitConverter.GetBytes(h);
